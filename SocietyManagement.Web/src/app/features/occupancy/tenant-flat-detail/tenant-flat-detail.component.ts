...
/** Full Tenant detail for one flat, reached by clicking a row in the
 * Tenants grid — tenant panel + emergency contacts + login accounts +
 * occupancy history + vehicles, all scoped to this flat. */
@Component({
  selector: 'app-tenant-flat-detail',
  standalone: true,
  imports: [
    CommonModule, PageHeaderComponent, TenantOccupancyPanelComponent, FlatEmergencyContactsCardComponent,
    FlatLoginCardComponent, OccupancyHistoryComponent, FlatVehiclesCardComponent
  ],
  template: `
    <div class="app-page">
      <app-page-header [title]="flatNumber() ? ('Flat ' + flatNumber() + ' — Tenant') : 'Tenant Detail'"
        subtitle="Tenant members, emergency contacts, login accounts, occupancy history, and vehicles for this flat."
        [breadcrumbs]="[{ label: 'Residents', link: '/residents' }, { label: 'Tenants', link: '/residents/tenants' }, { label: flatNumber() || '...' }]" />

      @if (societyId(); as sid) {
        <app-tenant-occupancy-panel [flatId]="flatId" [societyId]="sid" [occupancy]="overview()?.currentTenantOccupancy ?? null" (changed)="load()" />
        <app-flat-login-card [occupancy]="overview()?.currentTenantOccupancy ?? null" [flatId]="flatId" />
...
export class TenantFlatDetailComponent implements OnInit {
